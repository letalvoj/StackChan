...
| LED glow runs the wrong way when you stroke the head | Winding direction, which only the hardware knows. The two strips are wired in opposite directions and each has its own flag in `led_stage.h` — flip `kLeftStripRunsBackToFront` or `kRightStripRunsBackToFront` for whichever side chases backwards |
| Head-pet glow is too broad — touching the back lights the front too | Almost never the gaussian: at σ = 0.42 a pad reaches the far end of the head at ~0.001. It is the Si12T coupling into all three pads, so the far one reports 1–2 instead of 0. Raise `kTouchGamma` in `led_stage.cpp`, don't shrink σ. `LEDS.md` §4 |
...
| Pings answer, but every connect to 8081 is refused; face looks normal | The HTTP server never started. Boot log says `httpd_start failed: ESP_ERR_HTTPD_TASK` — no contiguous internal block for its 8 KB stack, and nothing retries it. Internal RAM, see the budget in `sdkconfig.defaults`. Do **not** move httpd's stack to PSRAM (`task_caps`): the JSON handler runs inline on it, and IDF asserts if a PSRAM-stacked task touches flash |
| Crash loop right after a config change, and reverting the change doesn't fix it | A Kconfig *choice* flipped as a side effect and stuck in the untracked `sdkconfig`. Toggling `SPIRAM_TRY_ALLOCATE_WIFI_LWIP` flips WiFi TX buffers to static (~26 KB internal) and they stay static after you toggle back. Diff `sdkconfig` against a fresh one generated from the defaults (`idf.py -B /tmp/x reconfigure` in a clean worktree) before blaming code |
| WebSocket opens but the device never sends `hello` | It adopts a client on its **first inbound frame**, not on the upgrade (this IDF never dispatches the handshake to the handler). Send your `hello` first; the device's arrives right after |
